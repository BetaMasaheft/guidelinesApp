        ...
        if (matches(request:get-header("User-Agent"), "^GitHub-Hookshot/")) then
          if (request:get-header("X-GitHub-Event") = "push") then
            ...
            let $expected-result := <expected-result>
              { request:get-header("X-Hub-Signature") }
            </expected-result>
            let $private-key := "Guid3l1n3s"
            ...
            let $condition := normalize-space($expected-result/text()) =
              normalize-space($actual-result/text())
            ...
            <response status="fail">
              <message>Invalid trigger.</message>
            </response>
        else
          <response status="fail">
            <message>This is not a GitHub request.</message>
          </response>
      } catch * {
        <response status="fail">
          <message>Unacceptable headers {
