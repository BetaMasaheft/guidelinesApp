...
declare variable $app:rng := doc(
  concat($config:app-root, "/schema/tei-betamesaheft.rng")
);

declare function app:modelorelem ($ec) {
  if (starts-with($ec, "tei_model")) then (
    for $contModel in $app:rng//rng:define[@name = $ec]//rng:ref/@name
    return app:modelorelem($contModel)
  ) else
    substring-after($ec, "tei_")
};
...
declare function app:queryinput (
  $node as node(),
  $model as map(*),
  $q as xs:string*
) {
  <input
    class="form-control diacritics"
    name="q"
...
(:~
 ...
 :)
declare %templates:wrap function app:hit-count (
  $node as node()*,
  $model as map(*)
) {
  ...
  return if (empty($q)) then (
      ...
      <h3>You found "{ $q }" in <span
          xmlns="http://www.w3.org/1999/xhtml"
          id="hit-count"
        >{ count($model("hits")) }</span> entries!</h3>
};
  ...
  if ($min-hits < 0 or count($model("hits")) >= $min-hits) then
    ...
    return (
        ...
        if ($start + $per-page < count($model("hits"))) then (
          <li>
            <a href="?{ $params }&amp;start={ $start + $per-page }">
          ...
          <li>
            ...
            ><i class="glyphicon glyphicon-fast-forward" /></a>
          </li>
        ) else (
          <li class="disabled">
...
function app:fullRes (
  ...
  return <div class="row">
      <a href="?{ $params }&amp;id={ data($id) }">{ $name }</a>
      <span class="badge pull-right">{ count($expanded//exist:match) }</span>
      <br />
      <div>{ kwic:summarize($term, <config width="40" />) }</div>
    </div>
};

declare function app:showitem (
  $node as node()*,
  $model as map(*),
  $id as xs:string?
) {
  ...
  return if ($id = "ontologyView") then (
      <div class="container-fluid">
        <div class="col-md-12">
          <div>
            ...
            <p>
              <a
                href="https://betamasaheft.github.io/RDF/"
              >OWLDoc Documentation</a>
            </p>
            <p>
              <a
                href="http://visualdataweb.de/webvowl/#url=https://raw.githubusercontent.com/BetaMasaheft/RDF/master/betamasaheft.json"
              >View in WebVOWL</a>
            </p>
          </div>
          <div>
            <p class="lead">The <i
              >Syntaxe du Codex</i> ontology  which we use for the description of the stratigraphy and history of manuscripts</p>
            <p>
              <a
                href="https://betamasaheft.github.io/SyntaxeDuCodex/"
              >OWLDoc Documentation</a>
            </p>
            <p>
              <a
                href="http://visualdataweb.de/webvowl/#url=https://raw.githubusercontent.com/BetaMasaheft/SyntaxeDuCodex/master/SyntaxeDuCodex.json"
              >View in WebVOWL</a>
            </p>
          </div>
        </div>
        ...
        <div class="col-md-12">
          {
            for $modRef in
              doc(
                "/db/apps/guidelines/schema/tei-betamesaheft.xml"
              )//tei:moduleRef
            return <div class="row-fluid alert alert-info">
                {
                  (
                    "Module " ||
                      string($modRef/@key) ||
                      " " ||
                      (
                        if ($modRef/@except) then (
                          "except: " || string($modRef/@except)
                        ) else (
                        )
                      )
                  )
        ...
        <div class="col-md-12">
          {
            ...
            order by $i
            return <div class="row alert alert-warning">
                ...
                <p class="lead">Module : { string($eleSpec/@module) }</p>
                {
                  transform:transform(
                    $eleSpec,
                    "xmldb:exist:///db/apps/guidelines/xslt/schemarules.xsl",
                    ()
                  ),
                  transform:transform(
                    $rngel,
      ...
      return <div class="container-fluid">
          <p>Th attribute <span class="label label-primary">
              { $id }
            </span> is mentioned in the following pages:</p>
          <ul>
            {
              ...
              let $name := $col//id(
                $R
              )/ancestor::tei:TEI//tei:titleStmt/tei:title/text()
              return <li><a href="/Guidelines/?id={ $R }">{ $name }</a></li>
            }
          </ul>
        </div>
    ) else if ($id) then (
      if ($term/name() = "body") then (
        <div class="container-fluid">
          <h2>{ $termName }</h2>
          {
            transform:transform(
              $term,
              "xmldb:exist:///db/apps/guidelines/xslt/text.xsl",
              ()
            )
          }
          {
            if (
              ...
              <div class="alert alert-info">
                <p class="lead">TEI guidelines <a
                    href="http://www.tei-c.org/release/doc/tei-p5-doc/en/html/ref-{
                      $id
                    }.html"
                    target="_blank"
                  >element { $id }</a>
              ...
              let $containerModel :=
              $app:rng//rng:define[descendant::rng:ref[@name =
                $teielement]]/@name
              let $elemContainer :=
              $app:rng//rng:define[descendant::rng:ref[@name =
                $containerModel]]/@name
              let $elnames :=
                for $ec in $elemContainer
                return app:modelorelem($ec)
              return <div class="alert alert-info">Contained by: 
{
                    for $e in distinct-values($elnames)
                    return <a class="badge" href="/Guidelines/?id={ $e }">
                        { $e }
                      </a>
                  }
                </div>,
              ...
              <div class="alert alert-success">
                <p>This element is mentioned in the following pages</p>
                <ul>
                  {
                    for $occurrence in
                      collection($config:data-root)//tei:gi[. = $id]
              ...
              <div class="alert alert-success">
                <p>This page is referred to in the following pages</p>
                <ul>
                  {
                    for $occurrence in
                      collection($config:data-root)//tei:ref[@target = $id]
                    let $ref := $occurrence/ancestor::tei:body/@xml:id
                    group by $R := $ref
                    let $name := $col//id(
                      $R
          ...
          <ul>
            {
              ...
              return <li>
                  { editors:editorKey($change/@who) } on {
                    string($change/@when)
                  }: { $change/text() }
                </li>
            }
        ...
        <div class="lead">Alas, there is no page yet with ID <span
            class="label label-primary"
          >
            { $id }
          </span>. If it is an element name, it might just need we do not have any page because there is no further specification to be given, so please look into the TEI guidelines for <a
            href="http://www.tei-c.org/release/doc/tei-p5-doc/en/html/ref-{
              $id
            }.html"
            target="_blank"
          >element { $id }</a>. Or do we need this page? <a
            class="btn btn-warning"
            href="https://github.com/BetaMasaheft/guidelines/issues/new?title=Please%20create%20page%20{
              $id
            }"
            target="_blank"
          >
      ...
      <div>
        ...
        <div>
          <p
          >You can find here the text of the guidelines for encoding in TEI information about the 
Ethiopic and Eritrean Manuscript Cultures, which is linked and presented for reference in connection with the Beta maṣāḥǝft Schema.</p>
          <p
          >We have tried to put as many examples as possible in the Schema as well as here in the Guidelines.</p>
          <p
          >These encoding practice guidelines as well as the schema are maintained and used by the Beta maṣāḥǝft project but are open to any interested user. 
We hope they will be used by other practitioners in the field for their purposes, for more and more projects.</p>
          <p>You can report problems and suggest changes in the <a
              href="https://github.com/BetaMasaheft/guidelines"
            >GitHub repository</a>.</p>
        </div>
        <div
          class="lead alert alert-info"
        >Browse and click on the left or Search and click on a search result to see it here.</div>
        <div class="lead alert alert-warning">Some Quick Links
<ul>
            <li><a href="/Guidelines?id=howto">Getting started</a></li>
            <li>
              <a href="/Guidelines?id=setup">Set up your working space</a>
              <a
                href="/Guidelines?id=workflow"
              >(with some help in using GitHub)</a>
            </li>
            <li>
              <a
                href="/Guidelines?id=general"
              >The general entry point to the guidelines</a>
            </li>
            <li><a href="/Guidelines?id=images">Images</a></li>
            <li><a href="/Guidelines?id=text-encoding">Text encoding</a></li>
            <li>
              <a
                href="/Guidelines?id=transliteration-principles"
              >Transliteration</a>
            </li>
    ...
    <ul class="nav nav-tabs">
      {
        ...
        return <li>
            {
              if (starts-with($name, "main")) then
                attribute class { "active" }
              else (
              )
            }
            <a data-toggle="tab" href="#toc{ $name }">
              { $toc//tei:titleStmt/tei:title/text() }
            </a>
          </li>
      }
    ...
    <div class="tab-content">
      {
        ...
        return <div id="toc{ $name }">
            {
              if (starts-with($name, "main")) then
                attribute class { "toc tab-pane fade in active" }
              else (
                attribute class { "toc tab-pane fade in" }
              )
            }
        ...
        <ul>
          {
            ...
            return <li><a href="/Guidelines/?id={ $id }">{ $name }</a></li>
          }
        </ul>
      </div>
    </div>
...
declare function app:tei2string ($nodes as node()*) {
  ...
  return typeswitch ($node)
      ...
      case element(tei:item) return
        if ($node/tei:list) then
          app:tei2string($node/node())
        else
          let $pageID := $node/text()
          let $page := collection($config:data-root)//id($pageID)
          let $id := if (count($page) gt 1) then
            "there are two pages with id " || $pageID || "!"
          else
            string($page/@xml:id)
          let $title := if (count($page) gt 1) then
            "there are two pages with id " || $pageID || "!"
          else
            $page/ancestor::tei:TEI//tei:titleStmt/tei:title/text()
          return <li><a href="/Guidelines/?id={ $id }">{ $title }</a></li>
      case element() return
        app:tei2string($node/node())
