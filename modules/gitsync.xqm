...
declare function gitsync:create-collections ($uri as xs:string) {
  ...
  return if (xmldb:collection-available($current-path)) then (
    ) else
...
(:~
 ...
 :)
declare function gitsync:do-update (
  $commits,
  $contents-url as xs:string?,
  $data-collection
) {
  let $committerEmail := $commits?1?committer?email
  return for $modified in $commits?1?modified?*
    ...
    return try {
        (
              ...
              <message>
                {
                  xmldb:store(
                    $collection-uri,
                    xmldb:encode-uri($file-name),
                    xs:base64Binary($file-data)
                  )
                }
              </message>
            </response>
          ...
          return gitsync:validateAndConfirm(
              $stored-file,
              $committerEmail,
              "updated"
            )
...
(:~
 ...
 :)
declare function gitsync:do-add (
  $commits,
  $contents-url as xs:string?,
  $data-collection
) {
  let $committerEmail := $commits?1?committer?email
  return for $modified in $commits?1?added?*
    let $file-path := concat($contents-url, $modified)
    let $gitToken := environment-variable("GITTOKEN")
    (: environment-variable('GIT_TOKEN') :)
    let $req := <http:request
              ...
              <message>
                {
                  (
                    ...
                    sm:chmod(
                      xs:anyURI(concat($collection-uri, "/", $file-name)),
                      "rwxrwxr-x"
                    ),
                    sm:chgrp(
                      xs:anyURI(concat($collection-uri, "/", $file-name)),
...
(:~
 ...
 : @param $contents-url string pointing to resource on github
 :)
declare function gitsync:do-delete (
  $commits,
  $contents-url as xs:string?,
  $data-collection
) {
  let $committerEmail := $commits?1?committer?email
  return for $modified in $commits?1?removed?*
    let $file-path := concat($contents-url, $modified)
    let $collection := xs:anyURI($data-collection)
    let $file-name := tokenize($modified, "/")[last()]
    let $resource-path := substring-before($modified, $file-name)
    let $collection-uri := replace(
      concat($collection, "/", $resource-path),
      "/$",
      ""
    )
    return try {
        <response status="okay">
          <message>removed { $file-name }
            { xmldb:remove($collection-uri, $file-name) }
          </message>
        </response>
      } catch * { ((: <response
                status="fail">
...
declare function gitsync:validateAndConfirm ($item, $mail, $type) {
  let $id := string($item//t:body/@xml:id)
  let $schema := xs:anyURI(
    "https://raw.githubusercontent.com/BetaMasaheft/guidelines/master/schema/out/tei-betamesaheftGL.rng"
  )
  (: validate :)
                    ...
                    <xhtml>
                        <html>
                            ...
                            <body>
                                ...
                                <p>This time however your newly created guidelines file <mark>{$id}.xml</mark>
                                    <b>did not validate</b> to the Beta maṣāḥǝft Guidelines schema.</p>
                                ...
                                <p><b>Please fix these issues as soon as possible and push the data to the <b>guidelines</b> repo again.</b></p>
...
declare function gitsync:wrongID ($mail, $storedFileID, $filename) {
  let $WrongIdMessage := <mail>
    ...
    <bcc />
    <subject
    >It looks like there is a mismatching id for a file you pushed to Beta maṣāḥǝft...</subject>
    <message>
      <xhtml>
        <html>
          <head>
            <title>The file { $filename }.xml has id {
                $storedFileID
              } instead of { $filename }.</title>
          </head>
          <body>
            <p>The file { $filename }.xml has id { $storedFileID } instead of {
                $filename
              }.</p>
            <p
            >
  ...
  return (: send the email
                ...
                then :) console:log("Sent Message to editor")
(: else
                    console:log('message not sent to editor') :)
};
...
(:~
 ...
 : this function is called only on add and update
 :)
declare function gitsync:failedCommitMessage (
  $mail,
  $data-collection,
  $message
) {
    ...
    <message>
      <xhtml>
        <html>
          ...
          <body>
            <p
            >Sorry, your latest push failed. It is not your fault and Pietro already knows about it and will try to fix it as soon as possible. Don't worry. Keep Calm and Carry On.</p>
            <p>{ $message }</p>
            <p>Thank you!</p>
          </body>
        </html>
      ...
      if ($json-data?ref = "refs/heads/master") then
        if (exists($json-data?commits)) then
          ...
          <response status="fail">
            <message
            >This is a GitHub request, however there were no commits.</message>
          </response>,
          gitsync:failedCommitMessage(
            "",
            $data-collection,
            "This is a GitHub request, however there were no commits."
          )
        )
      else (
        <response status="fail">
          <message>Not from the master branch.</message>
        </response>,
        gitsync:failedCommitMessage(
          "",
          $data-collection,
