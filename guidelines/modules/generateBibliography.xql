...
declare function local:zot($c) {
    let $xml-url-formattedBiblio := concat('https://api.zotero.org/groups/358366/items?tag=', $c, '&amp;format=bib&amp;style=hiob-ludolf-centre-for-ethiopian-studies&amp;linkwrap=1')
...
let $citations := distinct-values(collection('/db/apps/guidelines/data')//t:bibl/t:ptr/@target)
let $bibliography :=
<bibliography xmlns="dheth.biblio"><total>{count($citations)}</total>
    <entries>{
            for $c in $citations
            let $shortCit :=
            let $xml-url := concat('https://api.zotero.org/groups/358366/items?&amp;tag=', $c, '&amp;include=citation&amp;style=hiob-ludolf-centre-for-ethiopian-studies')
            let $req :=
            <http:request
                http-version="1.1"
...
return
   ...
   xmldb:store('/db/apps/guidelines/data', 'bibliography.xml', $bibliography)
